describe('Reservas - CRUD vía UI (mocked API)', () => {
  ...
  beforeEach(() => {
    ...
    cy.intercept('GET', '/api/reservas', (req) => {
      cy.get('@reservasState').then((reservas) => {
        req.reply({ statusCode: 200, body: reservas });
      });
    }).as('getReservas');
    ...
    cy.intercept('POST', '/api/reservas', (req) => {
      cy.get('@reservasState').then((reservas) => {
        ...
        reservas.push(nuevo);
        cy.wrap(reservas).as('reservasState');
    ...
    cy.intercept('PUT', /\/api\/reservas\/\d+/, (req) => {
      cy.get('@reservasState').then((reservas) => {
        ...
        if (idx !== -1) {
          ...
          cy.wrap(reservas).as('reservasState');
          req.reply({ statusCode: 200, body: reservas[idx] });
        } else {
  ...
  it('login, crear, editar y eliminar reserva', () => {
    ...
    cy.contains('Nuevo Reserva').click();
